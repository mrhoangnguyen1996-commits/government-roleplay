...
const SAVINGS_INTEREST_RATE = 0.005; // 0.5% mỗi kỳ — giảm chi trả lãi tiết kiệm so với trước
const LOAN_CYCLE_MS = 5 * 60 * 1000; // mô phỏng 1 "kỳ hạn" = 5 phút thực tế (RP time-compressed)
// v15.0: Lương cơ bản THEO NGÀY (cộng dồn tháng) + phụ cấp giờ trực + thưởng nghiệp vụ ca
...
const activeShifts = new Map(); // username -> { startAt, appsProcessed, registrations, displayName, role, agency }
const shiftHistory = []; // ca đã kết thúc
const dailySalaryLedger = {}; // username -> { 'dd/mm/yyyy': { basePay, hourPay, appPay, regPay, net, hours, ... } }
...
function computeShiftPay(record, officer) {
    // Trừ vi phạm chỉ áp dụng 1 lần ở cấp tháng (buildPayrollItems) — tránh trừ kép theo ca.
    const role = record.role || officer?.role || 'CHUYÊN VIÊN';
    const baseDaily = DAILY_BASE_SALARY[role] || DAILY_BASE_SALARY['CHUYÊN VIÊN'];
    const hours = Math.min(Math.max(record.hours || 0, 0), 12);
    const dutyRatio = Math.min(1, hours / 8);
    ...
    const net = Math.max(0, basePay + hourPay + appPay + regPay);
    return { basePay, hourPay, appPay, regPay, violationDeduction: 0, hours, appsProcessed: record.appsProcessed || 0, registrations: record.registrations || 0, net, role };
}
function getLiveShiftPay(username, officer) {
    const shift = activeShifts.get(username);
    if (!shift) return null;
    const hours = Math.round(((Date.now() - shift.startAt) / 3600000) * 100) / 100;
    const calc = computeShiftPay({ ...shift, hours }, officer || systemState.authorizedPersonnel[username]);
    return {
        date: todayKey(),
        ...calc,
        live: true,
...
function accumulateDailySalary(shiftRecord) {
    ...
    const ex = dailySalaryLedger[shiftRecord.username][dateKey] || { basePay: 0, hourPay: 0, appPay: 0, regPay: 0, violationDeduction: 0, net: 0, hours: 0, appsProcessed: 0, registrations: 0, shifts: 0, date: dateKey };
...
function buildPayrollItems(periodMonth, agencyFilter = null, options = {}) {
    ...
    return Object.values(systemState.authorizedPersonnel)
        ...
        .map(u => {
            ...
            const total = {
                basePay: 0, hourPay: 0, appPay: 0, regPay: 0, violationDeduction: 0, net: 0,
            ...
            Object.entries(ledger).forEach(([date, d]) => {
                ...
                if (parts.length >= 3 && `${parts[1]}/${parts[2]}` === monthKey) {
                    total.basePay += d.basePay; total.hourPay += d.hourPay; total.appPay += d.appPay; total.regPay += d.regPay;
            ...
            if (includeLive) {
                ...
                if (live) {
                    ...
                    if (inPeriod) {
                        ...
                        total.basePay += live.basePay; total.hourPay += live.hourPay; total.appPay += live.appPay; total.regPay += live.regPay;
            ...
            total.violationDeduction = (u.negativeRatings || 0) * 80000;
            total.net = Math.max(0, total.net - total.violationDeduction);
        ...
        .filter(item => includeZero || item.hours > 0 || item.workDays > 0 || item.onShift || item.finalNet > 0);
}
...
app.post('/api/approvals/pending', requireAuth(), safeRoute(async (req, res) => {
    ...
    const welfare = req.officer.role === 'ADMIN'
        ...
        : [];
    res.json({ success: true, payrollBatches: payroll, welfareBatches: welfare });
}));
...
app.post('/api/bank/staff-state', safeRoute(async (req, res) => {
    ...
    res.json({ success: true, accounts, transactions: bankTransactions.slice(0, 300), loans: loanRequests, accountRequests: bankAccountRequests, depositRequests: depositRequests.slice(0, 100), payrollBatches: payrollBatches.slice(0, 20), welfareBatches: welfareBatches.slice(0, 20), treasuryBalance: bankAccounts['__treasury__'].balance, interestRate: SAVINGS_INTEREST_RATE, payrollPeriod: currentMonthKey() });
}));
...
app.post('/api/bank/payroll/preview', safeRoute(async (req, res) => {
    ...
    res.json({
        ...
        autoSync: true,
        formula: 'LCB ngày (theo tỷ lệ giờ/8) + phụ cấp giờ trực + thưởng hồ sơ ca + thưởng nhập liệu − trừ vi phạm tháng',
...
app.post('/api/bank/payroll/submit', safeRoute(async (req, res) => {
    ...
    const period = req.body.period || currentMonthKey();
    // Tự động hóa: luôn tính lại từ sổ công + ca đang mở. Không nhận điều chỉnh tay từ lãnh đạo đơn vị.
    const items = buildPayrollItems(period, agency, { includeLive: true }).map(item => ({
        ...item,
    ...
    if (!items.length) return res.status(400).json({ error: `Đơn vị ${agency} chưa có dữ liệu chấm công/lương trong kỳ ${period}. Cán bộ cần vào ca và làm việc trước.` });
        ...
        batch.items.forEach(item => {
            ...
            if (acc && payAmount > 0) {
                ...
                logBankTx({
                    ...
                    note: `Kỳ ${batch.period} | LCB ngày: ${item.basePay.toLocaleString('vi-VN')} + Giờ trực: ${item.hourPay.toLocaleString('vi-VN')} + Hồ sơ: ${item.appPay.toLocaleString('vi-VN')} + Nhập liệu: ${item.regPay.toLocaleString('vi-VN')} - Trừ: ${item.violationDeduction.toLocaleString('vi-VN')}`,
                    time: batch.paidAt, status: 'THÀNH CÔNG', handledBy: actor.displayName,
                    payslip: { period: batch.period, basePay: item.basePay, hourPay: item.hourPay, appPay: item.appPay, regPay: item.regPay, violationDeduction: item.violationDeduction, workDays: item.workDays, hours: item.hours, appsProcessed: item.appsProcessed, registrations: item.registrations, net: payAmount, dailyBreakdown: item.dailyBreakdown, agency: batch.agency, approvedBy: actor.displayName }
                });
