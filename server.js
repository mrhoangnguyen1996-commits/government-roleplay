...
// VÀO CA = điểm danh thôi. Lương chỉ ghi sổ khi KẾT THÚC CA.
...
function computeShiftPay(record, officer) {
    // Chỉ dùng cho CA ĐÃ KẾT THÚC. Vào ca = điểm danh, chưa có tiền.
    // Lương ca = LCB theo tỷ lệ giờ/8 + phụ cấp giờ trực + thưởng HS/NL thực tế trong ca.
    const role = record.role || officer?.role || 'CHUYÊN VIÊN';
    const baseDaily = DAILY_BASE_SALARY[role] || DAILY_BASE_SALARY['CHUYÊN VIÊN'];
    const hours = Math.min(Math.max(Number(record.hours) || 0, 0), 12);
    if (hours <= 0) {
        return {
            basePay: 0, hourPay: 0, attendancePay: 0, appPay: 0, regPay: 0,
            violationDeduction: 0, hours: 0,
    ...
    return {
        basePay, hourPay, attendancePay: 0, appPay, regPay,
...
/** Theo dõi ca đang mở — KHÔNG cộng vào sổ lương / bảng chi. */
function getLiveShiftMonitor(username, officer) {
    ...
    return {
        date: todayKey(),
        hours,
        appsProcessed: shift.appsProcessed || 0,
        registrations: shift.registrations || 0,
        live: true,
        onShift: true,
        startAt: shift.startAt,
        elapsedMs: Date.now() - shift.startAt,
        displayName: shift.displayName || officer?.displayName,
        role: shift.role || officer?.role,
        agency: shift.agency || officer?.agency,
        // Ước tính tham khảo nếu kết ca ngay — không ghi sổ
        pendingEstimate: computeShiftPay({ ...shift, hours }, officer || systemState.authorizedPersonnel[username])
    };
}
function getLiveShiftPay(username, officer) {
    // Giữ alias cũ: chỉ trả monitor + estimate, không dùng để cộng bảng lương
...
function accumulateDailySalary(shiftRecord) {
    ...
    const ex = dailySalaryLedger[shiftRecord.username][dateKey] || {
        basePay: 0, hourPay: 0, attendancePay: 0, appPay: 0, regPay: 0, violationDeduction: 0,
        net: 0, hours: 0, appsProcessed: 0, registrations: 0, shifts: 0, date: dateKey, shiftIds: []
    };
...
function startOfficerShift(officer) {
    ...
    addLog(`🟢 Cán bộ ${officer.displayName} điểm danh VÀO CA trực ban [${officer.agency}] — chưa ghi nhận lương (chờ kết ca).`);
    return shift;
}
function endOfficerShift(username, reason = 'Kết thúc ca') {
    ...
    addLog(`🔴 ${shift.displayName} kết thúc ca (${reason}): ${hours}h | ${shift.appsProcessed} HS | ${shift.registrations} NL → GHI SỔ LƯƠNG ${pay.net.toLocaleString('vi-VN')} VND (tích lũy ngày: ${dayLedger.net.toLocaleString('vi-VN')} VND).`);
...
function buildPayrollItems(periodMonth, agencyFilter = null, options = {}) {
    ...
    // Hiện toàn bộ cán bộ ĐV để lãnh đạo theo dõi; số tiền CHỈ từ ca đã kết thúc (sổ dailySalaryLedger)
    ...
    return Object.values(systemState.authorizedPersonnel)
        ...
        .map(u => {
            ...
            Object.entries(ledger).forEach(([date, d]) => {
                ...
                if (parts.length >= 3 && `${parts[1]}/${parts[2]}` === monthKey) {
                    ...
                    total.dailyBreakdown.push({ date, ...d, live: false, onShift: false, recorded: true });
                }
            });
            if (live) {
                total.onShift = true;
            ...
            // Điều chỉnh biên bản / ứng lương (áp vào số đã ghi sổ)
            ...
            let net = recordedNet + total.bonusCredit - total.violationDeduction - total.advanceDebt;
            ...
            total.formulaNote = 'Chỉ ca đã kết thúc: LCB(giờ) + phụ cấp giờ + thưởng HS/NL + khen − phạt/VP − nợ ứng';
        ...
        .filter(item => includeZero || item.hours > 0 || item.workDays > 0 || item.finalNet > 0 || item.advanceDebt > 0 || item.bonusCredit > 0 || item.onShift);
}
...
function getOfficerIncome(username) {
    ...
    // Chỉ sổ ca đã kết thúc — không cộng ca đang mở vào lương
    const monthLedger = Object.entries(ledger).filter(([date]) => {
        const parts = date.split('/');
        return parts.length >= 3 && `${parts[1]}/${parts[2]}` === monthKey;
    }).map(([date, d]) => ({ date, ...d, live: false, onShift: false, recorded: true }));
    ...
    }), { net: 0, hours: 0, workDays: 0, shifts: 0 });
    ...
    return {
        ...
        pendingEstimate: live?.pendingEstimate?.net || 0,
        livePay: 0, // không cộng tiền khi đang ca
        adjustments: adj,
        advances: myAdvances,
        memos: myMemos,
        payslips,
...
app.post('/api/officer/attendance/checkout', requireAuth(), safeRoute(async (req, res) => {
    ...
    res.json({
        ...
        message: `Đã ghi sổ lương ca: ${result.pay.net.toLocaleString('vi-VN')} VND`
    });
}));
app.post('/api/officer/attendance/status', requireAuth(), safeRoute(async (req, res) => {
    ...
    res.json({
        success: true,
        attendance: status,
        // Chỉ sổ đã ghi nhận (ca kết thúc). Ca đang mở tách riêng.
...
app.post('/api/bank/payroll/preview', safeRoute(async (req, res) => {
    ...
    const recentClosed = shiftHistory
        .filter(s => (!agency || s.agency === agency) && (() => {
        ...
        .map(s => ({
            ...
            date: s.date, reason: s.reason, pay: computeShiftPay(s, systemState.authorizedPersonnel[s.username])
        }));
    res.json({
        success: true,
        items,
        ...
        autoSync: true,
        formula: 'Chỉ ca đã kết thúc: LCB(giờ/8) + phụ cấp giờ + thưởng HS/NL + khen − phạt/VP − nợ ứng. Vào ca = điểm danh, chưa cộng tiền.',
