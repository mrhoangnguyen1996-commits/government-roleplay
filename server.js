...
let payrollBatches = []; // { id, period, items[], status, submittedBy, approvedBy, time }
...
const BASE_SALARY = { 'CHUYÊN VIÊN': 5000000, 'LÃNH ĐẠO': 8000000, 'THANH TRA': 6500000, 'ADMIN': 10000000 };
...
function computeOfficerPayroll(officer) {
    const base = BASE_SALARY[officer.role] || BASE_SALARY['CHUYÊN VIÊN'];
    const approvedReports = systemState.shiftReports.filter(r => r.officer === officer.displayName && r.status === 'ĐÃ PHÊ DUYỆT').length;
    const kpiScore = Math.min(100, Math.max(0, 60 + officer.positiveRatings * 5 - officer.negativeRatings * 10 + approvedReports * 3));
    const bonus = Math.round(base * (kpiScore / 100) * 0.3) + officer.positiveRatings * 150000 + approvedReports * 100000;
    const violationDeduction = officer.negativeRatings * 400000;
    const net = Math.max(0, base + bonus - violationDeduction);
    return { base, bonus, violationDeduction, kpiScore, approvedReports, net };
}
function buildPayrollItems() {
    return Object.values(systemState.authorizedPersonnel)
        .filter(u => u.username !== '__treasury__' && u.role !== 'ADMIN')
        .map(u => {
            const calc = computeOfficerPayroll(u);
            return { username: u.username, displayName: u.displayName, role: u.role, agency: u.agency, ...calc };
        });
}
...
app.post('/api/bank/staff-state', safeRoute(async (req, res) => {
    ...
    res.json({ success: true, accounts, transactions: bankTransactions.slice(0, 300), loans: loanRequests, accountRequests: bankAccountRequests, depositRequests: depositRequests.slice(0, 100), payrollBatches: payrollBatches.slice(0, 20), treasuryBalance: bankAccounts['__treasury__'].balance, interestRate: SAVINGS_INTEREST_RATE });
}));

// [LÃNH ĐẠO NGÂN HÀNG] Xem trước bảng lương theo KPI
app.post('/api/bank/payroll/preview', safeRoute(async (req, res) => {
    ...
    res.json({ success: true, items: buildPayrollItems(), period: new Date().toLocaleDateString('vi-VN') });
}));

// [LÃNH ĐẠO NGÂN HÀNG] Trình bảng lương lên ADMIN phê duyệt
app.post('/api/bank/payroll/submit', safeRoute(async (req, res) => {
    ...
    if (payrollBatches.find(b => b.status === 'CHỜ ADMIN PHÊ DUYỆT')) {
        return res.status(409).json({ error: 'Đã có bảng lương đang chờ ADMIN phê duyệt.' });
    }
    const items = buildPayrollItems();
    const id = `BL-${Math.floor(1000 + Math.random() * 9000)}`;
    payrollBatches.unshift({ id, period: req.body.period || new Date().toLocaleDateString('vi-VN'), items, status: 'CHỜ ADMIN PHÊ DUYỆT', submittedBy: staff.displayName, time: new Date().toLocaleString('vi-VN'), totalNet: items.reduce((s, i) => s + i.net, 0) });
    addLog(`${staff.displayName} trình bảng lương [${id}] lên ADMIN phê duyệt — tổng chi: ${payrollBatches[0].totalNet.toLocaleString('vi-VN')} VND.`);
    res.json({ success: true, id });
}));

// [ADMIN] Phê duyệt bảng lương — chi trả vào tài khoản cán bộ
app.post('/api/bank/payroll/decide', safeRoute(async (req, res) => {
    ...
    if (!actor || actor.role !== 'ADMIN') return res.status(403).json({ error: 'Chỉ ADMIN mới phê duyệt chi lương.' });
    const batch = payrollBatches.find(b => b.id === req.body.id);
    if (!batch) return res.status(404).json({ error: 'Không tìm thấy bảng lương.' });
    if (batch.status !== 'CHỜ ADMIN PHÊ DUYỆT') return res.status(409).json({ error: 'Bảng lương đã được xử lý.' });
    const decision = req.body.decision;
    if (decision === 'DUYỆT') {
        ...
        batch.items.forEach(item => {
            ...
            if (acc && item.net > 0) {
                acc.balance += item.net;
                const txId = `LUONG-${Date.now().toString().slice(-6)}-${item.username.slice(0, 3)}`;
                logBankTx({ id: txId, accountNumber: acc.accountNumber, type: 'CHI TRẢ LƯƠNG & PHÚC LỢI', amount: item.net, counterpart: 'KHO BẠC NHÀ NƯỚC', note: `Kỳ ${batch.period} | LCB: ${item.base.toLocaleString('vi-VN')} + Thưởng KPI: ${item.bonus.toLocaleString('vi-VN')} - Trừ VP: ${item.violationDeduction.toLocaleString('vi-VN')}`, time: batch.paidAt, status: 'THÀNH CÔNG', handledBy: actor.displayName, payslip: { base: item.base, bonus: item.bonus, violationDeduction: item.violationDeduction, kpiScore: item.kpiScore, net: item.net, period: batch.period } });
                item.paid = true; item.txId = txId;
            } else {
                item.paid = false; item.note = acc ? 'Số tiền bằng 0' : 'Chưa có tài khoản ngân hàng';
            }
        });
        bankAccounts['__treasury__'].balance = Math.max(0, (bankAccounts['__treasury__'].balance || 0) - batch.totalNet);
        addLog(`ADMIN ${actor.displayName} PHÊ DUYỆT chi lương [${batch.id}] — ${batch.items.filter(i => i.paid).length} cán bộ nhận lương.`);
    } else {
