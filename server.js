...
let loanRequests = []; // { id, accountNumber, username, amount, reason, status, time, decidedBy }
...
app.post('/api/bank/issue-account', safeRoute(async (req, res) => {
    ...
    if (!staff) return res.status(403).json({ error: 'Chỉ Nhân Viên/Lãnh Đạo Ngân Hàng hoặc ADMIN mới có quyền cấp tài khoản.' });
...
app.post('/api/bank/pay-fine', safeRoute(async (req, res) => {
    ...
    addLog(`Công dân @${acc.username} nộp phạt ${amount.toLocaleString('vi-VN')} VND cho biên bản [${violationId}] qua Ngân Hàng Số.`);
    broadcastUpdate();
    res.json({ success: true, balance: acc.balance, billId: id });
}));
...
app.post('/api/bank/loan/decide', safeRoute(async (req, res) => {
    ...
    loan.status = decision === 'DUYỆT' ? 'ĐÃ GIẢI NGÂN' : 'TỪ CHỐI'; loan.decidedBy = staff.displayName;
    if (decision === 'DUYỆT') {
        const acc = bankAccounts[loan.username];
        if (acc) {
            acc.balance += loan.amount;
            logBankTx({ id: `GD-${Date.now().toString().slice(-8)}`, accountNumber: acc.accountNumber, type: 'GIẢI NGÂN VAY VỐN', amount: loan.amount, counterpart: 'NGÂN HÀNG', note: loan.reason, time: new Date().toLocaleString('vi-VN'), status: 'THÀNH CÔNG' });
        }
    }
...
app.post('/api/bank/staff-state', safeRoute(async (req, res) => {
    ...
    res.json({ success: true, accounts, transactions: bankTransactions.slice(0, 300), loans: loanRequests, treasuryBalance: bankAccounts['__treasury__'].balance });
}));
...
server.listen(PORT, () => {
    console.log(`[REALTIME CENTRAL V11.0] RUNNING PERFECTLY ON PORT ${PORT}`);
