...
function buildPayrollItems(periodMonth, agencyFilter = null) {
...
app.post('/api/bank/payroll/preview', safeRoute(async (req, res) => {
    ...
    const items = buildPayrollItems(period, agency);
    res.json({ success: true, items, period, agency: agency || 'TOÀN QUỐC', seal: getAgencySeal(agency || actor.agency) });
}));

// [LÃNH ĐẠO ĐƠN VỊ] Ký đề nghị chi lương cho đơn vị mình → chuyển Ngân Hàng kiểm tra
app.post('/api/bank/payroll/submit', safeRoute(async (req, res) => {
    ...
    const items = buildPayrollItems(period, agency).map(item => {
        const proposedNet = adjustments[item.username] !== undefined ? Math.max(0, parseInt(adjustments[item.username], 10) || 0) : item.finalNet;
        return { ...item, proposedNet, unitLeaderAdjusted: proposedNet !== item.finalNet, unitLeaderNote: req.body.note || '' };
    });
    if (!items.length) return res.status(400).json({ error: `Đơn vị ${agency} chưa có dữ liệu chấm công/lương trong kỳ ${period}.` });
    ...
    payrollBatches.unshift({
        ...
        workflow: [{ step: 'LÃNH ĐẠO ĐƠN VỊ KÝ ĐỀ NGHỊ CHI LƯƠNG', by: actor.displayName, time, note: req.body.note || 'Đã rà soát ngày công và đề nghị chi lương.' }],
        ...
        proposalForm: { title: `Đề nghị chi lương đơn vị ${agency}`, note: req.body.note || '', period }
    });
    addLog(`${actor.displayName} ký đề nghị chi lương [${id}] cho đơn vị [${agency}] kỳ ${period} — chờ Ngân Hàng kiểm tra.`);
    broadcastUpdate();
    res.json({ success: true, id, agency });
}));
