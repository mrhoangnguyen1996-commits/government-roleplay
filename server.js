...
function computeShiftPay(record, officer) {
    ...
    const violationDeduction = Math.round(((officer?.negativeRatings || 0) * 80000) / 26);
    const net = Math.max(0, basePay + hourPay + appPay + regPay - violationDeduction);
    return { basePay, hourPay, appPay, regPay, violationDeduction, hours, appsProcessed: record.appsProcessed || 0, registrations: record.registrations || 0, net, role };
}
function accumulateDailySalary(shiftRecord) {
    ...
    dailySalaryLedger[shiftRecord.username][dateKey] = {
        ...
        violationDeduction: calc.violationDeduction,
        net: ex.net + calc.net,
...
function buildPayrollItems(periodMonth) {
    ...
    return Object.values(systemState.authorizedPersonnel)
        ...
        .map(u => {
            ...
            const total = { basePay: 0, hourPay: 0, appPay: 0, regPay: 0, violationDeduction: 0, net: 0, hours: 0, appsProcessed: 0, registrations: 0, workDays: 0, dailyBreakdown: [] };
            Object.entries(ledger).forEach(([date, d]) => {
                ...
                if (parts.length >= 3 && `${parts[1]}/${parts[2]}` === monthKey) {
                    ...
                    total.registrations += d.registrations; total.workDays += 1;
                    total.dailyBreakdown.push({ date, ...d });
                }
            });
            total.violationDeduction = u.negativeRatings * 80000;
...
app.post('/api/officer/attendance/status', requireAuth(), safeRoute(async (req, res) => {
    ...
    const today = dailySalaryLedger[req.officer.username]?.[todayKey()] || null;
    res.json({ success: true, attendance: status, todayLedger: today, recentShifts: shiftHistory.filter(s => s.username === req.officer.username).slice(0, 10) });
}));
...
app.post('/api/approvals/pending', requireAuth(), safeRoute(async (req, res) => {
    const payroll = payrollBatches.filter(b => !['ĐÃ CHI TRẢ', 'TỪ CHỐI'].includes(b.status)).slice(0, 20);
...
// [LÃNH ĐẠO NGÂN HÀNG] Xem trước bảng lương theo điểm danh + nghiệp vụ ca trực (cộng dồn tháng)
app.post('/api/bank/payroll/preview', safeRoute(async (req, res) => {
    const staff = verifyBankStaff(req.body.actorUsername, req.body.actorPassword);
    if (!staff || (staff.role !== 'LÃNH ĐẠO' && staff.role !== 'ADMIN')) {
        return res.status(403).json({ error: 'Chỉ Lãnh Đạo Ngân Hàng hoặc ADMIN mới lập bảng lương.' });
    }
    const period = req.body.period || currentMonthKey();
    res.json({ success: true, items: buildPayrollItems(period), period });
}));

// [LÃNH ĐẠO NGÂN HÀNG] Trình bảng lương → Lãnh Đạo cơ quan phê duyệt (bước 1)
app.post('/api/bank/payroll/submit', safeRoute(async (req, res) => {
    const staff = verifyBankStaff(req.body.actorUsername, req.body.actorPassword);
    if (!staff || (staff.role !== 'LÃNH ĐẠO' && staff.role !== 'ADMIN')) {
        return res.status(403).json({ error: 'Chỉ Lãnh Đạo Ngân Hàng mới trình bảng lương.' });
    }
    const pending = payrollBatches.find(b => ['CHỜ LÃNH ĐẠO CƠ QUAN', 'CHỜ LÃNH ĐẠO NGÂN HÀNG', 'CHỜ ADMIN PHÊ DUYỆT'].includes(b.status));
    if (pending) return res.status(409).json({ error: `Đã có bảng lương [${pending.id}] đang chờ phê duyệt.` });
    const period = req.body.period || currentMonthKey();
    const items = buildPayrollItems(period);
    const id = `BL-${Math.floor(1000 + Math.random() * 9000)}`;
    payrollBatches.unshift({
        id, period, items, status: 'CHỜ LÃNH ĐẠO CƠ QUAN',
        workflow: [{ step: 'LÃNH ĐẠO NGÂN HÀNG LẬP', by: staff.displayName, time: new Date().toLocaleString('vi-VN'), note: 'Trình duyệt lương theo ngày' }],
        submittedBy: staff.displayName, time: new Date().toLocaleString('vi-VN'),
        totalNet: items.reduce((s, i) => s + i.finalNet, 0)
    });
    addLog(`${staff.displayName} trình bảng lương [${id}] kỳ ${period} — chờ Lãnh Đạo cơ quan phê duyệt.`);
    res.json({ success: true, id });
}));

// Phê duyệt đa cấp: Lãnh Đạo cơ quan → Lãnh Đạo Ngân Hàng → ADMIN (điều chỉnh cuối)
app.post('/api/bank/payroll/approve-step', safeRoute(async (req, res) => {
    ...
    if (batch.status === 'CHỜ LÃNH ĐẠO CƠ QUAN') {
        if (actor.role !== 'LÃNH ĐẠO' && actor.role !== 'ADMIN' && actor.role !== 'THANH TRA') {
            return res.status(403).json({ error: 'Chỉ Lãnh Đạo/Thanh Tra mới phê duyệt bước 1.' });
        }
        batch.status = 'CHỜ LÃNH ĐẠO NGÂN HÀNG';
        batch.workflow.push({ step: 'LÃNH ĐẠO CƠ QUAN DUYỆT', by: actor.displayName, time: new Date().toLocaleString('vi-VN'), note });
        addLog(`${actor.displayName} phê duyệt bước 1 bảng lương [${batch.id}] → chuyển Ngân Hàng.`);
    } else if (batch.status === 'CHỜ LÃNH ĐẠO NGÂN HÀNG') {
        ...
        if (!bankLead || (bankLead.role !== 'LÃNH ĐẠO' && bankLead.role !== 'ADMIN')) {
            return res.status(403).json({ error: 'Chỉ Lãnh Đạo Ngân Hàng mới phê duyệt bước 2.' });
        }
        batch.status = 'CHỜ ADMIN PHÊ DUYỆT';
        batch.workflow.push({ step: 'LÃNH ĐẠO NGÂN HÀNG DUYỆT', by: bankLead.displayName, time: new Date().toLocaleString('vi-VN'), note });
        addLog(`${bankLead.displayName} phê duyệt bước 2 bảng lương [${batch.id}] → trình ADMIN.`);
    } else {
    ...
    if (decision === 'DUYỆT') {
        batch.items.forEach(item => {
            if (adjustments[item.username] !== undefined) {
                ...
                item.adjustedNet = item.finalNet;
            }
        });
        batch.totalNet = batch.items.reduce((s, i) => s + (i.adjustedNet ?? i.finalNet), 0);
        ...
        batch.items.forEach(item => {
            const payAmount = item.adjustedNet ?? item.finalNet;
...
server.listen(PORT, () => {
    ...
    console.log(`[LƯƠNG v15] Điểm danh ca trực + lương theo ngày + phê duyệt 3 cấp + phúc lợi cộng đồng: SẴN SÀNG.`);
