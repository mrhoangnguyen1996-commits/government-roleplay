...
const SHIFT_CHECKIN_BONUS = 50000; // thưởng điểm danh vào ca (không cần nhập liệu)
...
function computeShiftPay(record, officer) {
    // Lương ca = LCB theo giờ trực + phụ cấp giờ + thưởng điểm danh + thưởng HS/NL
    // Không bắt buộc nhập liệu mới có lương — chỉ cần vào ca là đã tính theo thời gian.
    const role = record.role || officer?.role || 'CHUYÊN VIÊN';
    const baseDaily = DAILY_BASE_SALARY[role] || DAILY_BASE_SALARY['CHUYÊN VIÊN'];
    const rawHours = Math.max(record.hours || 0, 0);
    const hours = Math.min(rawHours, 12);
    // Tối thiểu 0.1h khi đã vào ca (tránh ca mới mở hiện 0đ gây hiểu nhầm)
    const billableHours = record.onShiftLive && hours < 0.1 ? 0.1 : hours;
    const dutyRatio = Math.min(1, Math.max(billableHours, 0.1) / 8);
    const basePay = Math.round(baseDaily * dutyRatio);
    const hourPay = Math.round(billableHours * (HOURLY_DUTY_RATE[role] || HOURLY_DUTY_RATE['CHUYÊN VIÊN']));
    const checkinPay = (record.onShiftLive || record.hours > 0) ? SHIFT_CHECKIN_BONUS : 0;
    // Chỉ cộng thưởng điểm danh 1 lần/ca khi kết thúc hoặc đang live
    const appPay = (record.appsProcessed || 0) * BONUS_PER_APP_SHIFT;
    const regPay = (record.registrations || 0) * BONUS_PER_REG_SHIFT;
    const attendancePay = checkinPay;
    const net = Math.max(0, basePay + hourPay + attendancePay + appPay + regPay);
    return {
        basePay, hourPay, attendancePay, appPay, regPay,
        violationDeduction: 0,
        hours: Math.round(billableHours * 100) / 100,
        appsProcessed: record.appsProcessed || 0,
        registrations: record.registrations || 0,
        net, role
    };
}
function getPayAdj(username) {
    if (!officerPayAdjustments[username]) {
        officerPayAdjustments[username] = { advanceDebt: 0, bonusCredit: 0, penaltyDebt: 0 };
    }
...
function getLiveShiftPay(username, officer) {
    ...
    const calc = computeShiftPay({ ...shift, hours, onShiftLive: true }, officer || systemState.authorizedPersonnel[username]);
    return {
        date: todayKey(),
        ...calc,
        live: true,
...
function accumulateDailySalary(shiftRecord) {
    ...
    const calc = computeShiftPay(shiftRecord, officer);
    const dateKey = shiftRecord.date || todayKey();
    if (!dailySalaryLedger[shiftRecord.username]) dailySalaryLedger[shiftRecord.username] = {};
    const ex = dailySalaryLedger[shiftRecord.username][dateKey] || { basePay: 0, hourPay: 0, attendancePay: 0, appPay: 0, regPay: 0, violationDeduction: 0, net: 0, hours: 0, appsProcessed: 0, registrations: 0, shifts: 0, date: dateKey };
    dailySalaryLedger[shiftRecord.username][dateKey] = {
        basePay: ex.basePay + calc.basePay,
        hourPay: ex.hourPay + calc.hourPay,
        attendancePay: (ex.attendancePay || 0) + (calc.attendancePay || 0),
        appPay: ex.appPay + calc.appPay,
        regPay: ex.regPay + calc.regPay,
        violationDeduction: 0,
        net: ex.net + calc.net,
        hours: Math.round((ex.hours + calc.hours) * 100) / 100,
        appsProcessed: ex.appsProcessed + calc.appsProcessed,
        registrations: ex.registrations + calc.registrations,
        shifts: ex.shifts + 1,
        date: dateKey
    };
}
function startOfficerShift(officer) {
    if (activeShifts.has(officer.username)) return activeShifts.get(officer.username);
    const shift = { username: officer.username, displayName: officer.displayName, role: officer.role, agency: officer.agency, startAt: Date.now(), appsProcessed: 0, registrations: 0 };
    activeShifts.set(officer.username, shift);
    addLog(`🟢 Cán bộ ${officer.displayName} điểm danh VÀO CA trực ban [${officer.agency}].`);
    return shift;
}
function endOfficerShift(username, reason = 'Kết thúc ca') {
    ...
    const hours = Math.round(((endAt - shift.startAt) / 3600000) * 100) / 100;
    const record = { ...shift, endAt, hours, date: todayKey(), id: `CA-${Date.now().toString().slice(-8)}`, reason };
    shiftHistory.unshift(record);
    if (shiftHistory.length > 5000) shiftHistory.pop();
    activeShifts.delete(username);
    accumulateDailySalary(record);
    addLog(`🔴 ${shift.displayName} kết thúc ca (${reason}): ${hours}h | ${shift.appsProcessed} hồ sơ | ${shift.registrations} nhập liệu cư dân.`);
    return record;
}
function buildPayrollItems(periodMonth, agencyFilter = null, options = {}) {
    const monthKey = periodMonth || currentMonthKey();
    // Mặc định hiện toàn bộ cán bộ đơn vị (kể cả chưa vào ca) để lãnh đạo theo dõi — không báo "chưa có dữ liệu" vô lý
    ...
    return Object.values(systemState.authorizedPersonnel)
        ...
        .map(u => {
            ...
            const total = {
                basePay: 0, hourPay: 0, attendancePay: 0, appPay: 0, regPay: 0, violationDeduction: 0, net: 0,
                hours: 0, appsProcessed: 0, registrations: 0, workDays: 0, dailyBreakdown: [],
                onShift: false, liveHours: 0, closedHours: 0, hasBankAccount: !!bankAccounts[u.username],
            ...
            Object.entries(ledger).forEach(([date, d]) => {
                ...
                if (parts.length >= 3 && `${parts[1]}/${parts[2]}` === monthKey) {
                    total.basePay += d.basePay; total.hourPay += d.hourPay;
                    total.attendancePay += (d.attendancePay || 0);
                    ...
                    total.dailyBreakdown.push({ date, ...d, live: false, onShift: false });
                }
            });
            if (includeLive) {
                const live = getLiveShiftPay(u.username, u);
                if (live) {
                    const parts = live.date.split('/');
                    const inPeriod = parts.length >= 3 && `${parts[1]}/${parts[2]}` === monthKey;
                    if (inPeriod) {
                        total.onShift = true;
                        total.statusLabel = 'ĐANG TRỰC CA';
                        total.liveHours = live.hours;
                        total.basePay += live.basePay; total.hourPay += live.hourPay;
                        total.attendancePay += (live.attendancePay || 0);
                        total.appPay += live.appPay; total.regPay += live.regPay;
                        total.net += live.net; total.hours += live.hours; total.appsProcessed += live.appsProcessed;
                        total.registrations += live.registrations;
                        const idx = total.dailyBreakdown.findIndex(d => d.date === live.date);
                        if (idx >= 0) {
                            const closed = total.dailyBreakdown[idx];
                            total.dailyBreakdown[idx] = {
                                date: live.date,
                                basePay: closed.basePay + live.basePay,
                                hourPay: closed.hourPay + live.hourPay,
                                attendancePay: (closed.attendancePay || 0) + (live.attendancePay || 0),
                                appPay: closed.appPay + live.appPay,
                                regPay: closed.regPay + live.regPay,
                                violationDeduction: 0,
                                net: closed.net + live.net,
                                hours: Math.round((closed.hours + live.hours) * 100) / 100,
                                appsProcessed: closed.appsProcessed + live.appsProcessed,
                                registrations: closed.registrations + live.registrations,
                                shifts: closed.shifts || 0,
                                live: true, onShift: true,
                                closedHours: closed.hours, liveHours: live.hours, startAt: live.startAt
                            };
                        } else {
                            total.workDays += 1;
                            total.dailyBreakdown.push(live);
                        }
                    }
                }
            }
            if (!total.onShift && total.workDays > 0) total.statusLabel = 'ĐÃ CÓ CÔNG';
            // Trừ vi phạm đánh giá + phạt biên bản; cộng thưởng biên bản; trừ nợ ứng lương
            total.violationDeduction = ((u.negativeRatings || 0) * 80000) + (adj.penaltyDebt || 0);
            total.bonusCredit = adj.bonusCredit || 0;
            total.advanceDebt = adj.advanceDebt || 0;
            let net = total.net + total.bonusCredit - total.violationDeduction - total.advanceDebt;
            ...
            total.formulaNote = 'LCB(giờ) + phụ cấp giờ + thưởng vào ca + thưởng HS/NL + khen − phạt/VP − nợ ứng';
        ...
        .filter(item => includeZero || item.hours > 0 || item.workDays > 0 || item.onShift || item.finalNet > 0 || item.advanceDebt > 0 || item.bonusCredit > 0);
}
...
function getUnitRoster(agency) {
    return Object.values(systemState.authorizedPersonnel)
        ...
        .map(u => {
            ...
            const live = getLiveShiftPay(u.username, u);
            const today = (dailySalaryLedger[u.username] || {})[todayKey()] || null;
            return {
                username: u.username,
                ...
                appsProcessed: att.appsProcessed || 0,
                registrations: att.registrations || 0,
                livePay: live ? live.net : 0,
...
function getOfficerIncome(username) {
    ...
    const monthLedger = Object.entries(ledger).filter(([date]) => {
        const parts = date.split('/');
        return parts.length >= 3 && `${parts[1]}/${parts[2]}` === monthKey;
    }).map(([date, d]) => ({ date, ...d, live: false, onShift: false }));
    const live = getLiveShiftPay(username, officer);
    if (live) {
        const idx = monthLedger.findIndex(d => d.date === live.date);
        if (idx >= 0) {
            const closed = monthLedger[idx];
            monthLedger[idx] = {
                date: live.date,
                basePay: closed.basePay + live.basePay,
                hourPay: closed.hourPay + live.hourPay,
    ...
    const monthTotal = monthLedger.reduce((s, d) => ({ net: s.net + d.net, hours: s.hours + d.hours, workDays: s.workDays + 1 }), { net: 0, hours: 0, workDays: 0 });
...
function getAttendanceStatus(username) {
    const shift = activeShifts.get(username);
    if (!shift) return { onShift: false };
    return { onShift: true, startAt: shift.startAt, appsProcessed: shift.appsProcessed, registrations: shift.registrations, elapsedMs: Date.now() - shift.startAt };
}
...
app.post('/api/officer/attendance/checkin', requireAuth(), safeRoute(async (req, res) => {
    ...
    broadcastUpdate();
    res.json({ success: true, shift: { startAt: shift.startAt, onShift: true } });
}));
app.post('/api/officer/attendance/checkout', requireAuth(), safeRoute(async (req, res) => {
    const record = endOfficerShift(req.officer.username, req.body.reason || 'Kết thúc ca thủ công');
    if (!record) return res.status(409).json({ error: 'Bạn chưa điểm danh vào ca, không thể kết thúc ca.' });
    const pay = computeShiftPay(record, req.officer);
    broadcastUpdate();
    res.json({ success: true, record, dailyPay: pay });
}));
app.post('/api/officer/attendance/status', requireAuth(), safeRoute(async (req, res) => {
    ...
    const todayClosed = dailySalaryLedger[req.officer.username]?.[todayKey()] || null;
    const live = getLiveShiftPay(req.officer.username, req.officer);
    const todayLedger = live ? {
        basePay: (todayClosed?.basePay || 0) + live.basePay,
        hourPay: (todayClosed?.hourPay || 0) + live.hourPay,
        attendancePay: (todayClosed?.attendancePay || 0) + (live.attendancePay || 0),
        appPay: (todayClosed?.appPay || 0) + live.appPay,
        regPay: (todayClosed?.regPay || 0) + live.regPay,
        net: (todayClosed?.net || 0) + live.net,
        hours: Math.round(((todayClosed?.hours || 0) + live.hours) * 100) / 100,
        appsProcessed: (todayClosed?.appsProcessed || 0) + live.appsProcessed,
        registrations: (todayClosed?.registrations || 0) + live.registrations,
        live: true
    } : todayClosed;
    res.json({ success: true, attendance: status, todayLedger, recentShifts: shiftHistory.filter(s => s.username === req.officer.username).slice(0, 10) });
}));
...
app.post('/api/officer/unit-roster', requireAuth(), safeRoute(async (req, res) => {
    ...
    const payroll = buildPayrollItems(currentMonthKey(), agency, { includeLive: true });
    res.json({
        success: true,
        agency,
...
app.post('/api/officer/advance/submit', requireAuth(), safeRoute(async (req, res) => {
    ...
    const earned = buildPayrollItems(currentMonthKey(), req.officer.agency, { includeLive: true, includeZero: true })
...
app.post('/api/bank/payroll/preview', safeRoute(async (req, res) => {
    ...
    const items = buildPayrollItems(period, agency, { includeLive: true });
    ...
    res.json({
        ...
        autoSync: true,
        formula: 'LCB(giờ) + phụ cấp giờ + thưởng vào ca + thưởng HS/NL + khen thưởng − phạt/VP − nợ ứng lương',
        onShiftCount: items.filter(i => i.onShift).length,
        withPayCount: items.filter(i => i.hours > 0 || i.onShift || i.finalNet > 0).length,
...
app.post('/api/bank/payroll/submit', safeRoute(async (req, res) => {
    ...
    const period = req.body.period || currentMonthKey();
    // Preview hiện cả cán bộ chưa vào ca; khi ký chỉ đưa vào batch những người đã có công / đang ca / có điều chỉnh tiền.
    const items = buildPayrollItems(period, agency, { includeLive: true, includeZero: false })
        .filter(item => item.hours > 0 || item.onShift || item.finalNet > 0 || item.advanceDebt > 0 || item.bonusCredit > 0)
        .map(item => ({
            ...item,
    ...
    if (!items.length) return res.status(400).json({ error: `Đơn vị ${agency} chưa có cán bộ nào vào ca / có công trong kỳ ${period}. Cán bộ chỉ cần điểm danh vào ca là hệ thống đã ghi nhận lương theo giờ trực.` });
    const onShiftNames = items.filter(i => i.onShift).map(i => i.displayName);
    const id = `BL-${Math.floor(1000 + Math.random() * 9000)}`;
    const unitSeal = getAgencySeal(agency);
    const time = new Date().toLocaleString('vi-VN');
    const noteParts = [req.body.note || 'Đã rà soát bảng lương tự động đồng bộ từ ca trực.'];
    if (onShiftNames.length) noteParts.push(`Snapshot gồm ${onShiftNames.length} ca đang mở: ${onShiftNames.join(', ')}.`);
    payrollBatches.unshift({
        id, period, agency, items, status: 'CHỜ NGÂN HÀNG KIỂM TRA',
        workflow: [{ step: 'LÃNH ĐẠO ĐƠN VỊ KÝ ĐỀ NGHỊ CHI LƯƠNG (TỰ ĐỘNG)', by: actor.displayName, time, note: noteParts.join(' ') }],
        submittedBy: actor.displayName, time,
        totalNet: items.reduce((s, i) => s + (i.proposedNet ?? i.finalNet), 0),
        autoCalculated: true,
        includesLiveShifts: onShiftNames.length > 0,
        signatures: {
        ...
        proposalForm: { title: `Đề nghị chi lương tự động — đơn vị ${agency}`, note: noteParts.join(' '), period }
    });
    addLog(`${actor.displayName} ký đề nghị chi lương TỰ ĐỘNG [${id}] cho đơn vị [${agency}] kỳ ${period} — ${items.length} cán bộ, tổng ${items.reduce((s, i) => s + i.finalNet, 0).toLocaleString('vi-VN')} VND.`);
    broadcastUpdate();
    res.json({ success: true, id, agency, count: items.length, includesLiveShifts: onShiftNames.length > 0 });
}));
            ...
            if (acc && payAmount > 0) {
                ...
                logBankTx({
                    ...
                    payslip: {
                        ...
                        attendancePay: item.attendancePay || 0, appPay: item.appPay, regPay: item.regPay,
...
io.on('connection', (socket) => {
    ...
    socket.on('logout', () => {
        ...
        if (info) endOfficerShift(info.username, 'Đăng xuất hệ thống');
    ...
    socket.on('disconnect', () => {
        ...
        broadcastVisitorCount();
        const info = onlineOfficers.get(socket.id);
        if (info) endOfficerShift(info.username, 'Ngắt kết nối / đóng trang');
        if (onlineOfficers.has(socket.id)) {
            onlineOfficers.delete(socket.id);
            broadcastRoster();
        }
