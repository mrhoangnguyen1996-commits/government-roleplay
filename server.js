...
let systemState = {
    ...
    authorizedPersonnel: {
        "admin": { username: "admin", password: "123", displayName: "Văn Phòng Trung Ương Điều Hành", role: "ADMIN", agency: "HỆ THỐNG", positiveRatings: 0, negativeRatings: 0, baseSalary: 0 },
        "canbo1": { username: "canbo1", password: "123", displayName: "Thiếu Tá Lê Minh Tuấn", role: "CHUYÊN VIÊN", agency: "CÔNG AN", positiveRatings: 5, negativeRatings: 1, baseSalary: 12000000 },
        "lanhdao1": { username: "lanhdao1", password: "123", displayName: "Đại Tá Nguyễn Hoàng", role: "LÃNH ĐẠO", agency: "CÔNG AN", positiveRatings: 8, negativeRatings: 0, baseSalary: 25000000 },
        "thanhtra1": { username: "thanhtra1", password: "123", displayName: "Thanh Tra Viên Trần Lực", role: "THANH TRA", agency: "THANH TRA", positiveRatings: 2, negativeRatings: 0, baseSalary: 18000000 },
        "nganhang1": { username: "nganhang1", password: "123", displayName: "Phó Giám Đốc Ngân Hàng Trần Minh", role: "LÃNH ĐẠO", agency: "NGÂN HÀNG", positiveRatings: 3, negativeRatings: 0, baseSalary: 30000000 }
    }
};
...
let depositRequests = []; // { id, accountNumber, username, amount, method, note, status, time, decidedBy }
let salaryPayrollRuns = []; // { id, period, status, createdBy, approvedBy, time, entries: [{ username, displayName, baseSalary, bonus, deduction, kpiScore, total, paid }] }
const SAVINGS_INTEREST_RATE = 0.004; // 0.4% mỗi kỳ — giảm mức chi trả lãi tiết kiệm
const BASE_SALARY_DEFAULT = 10000000;
...
function verifyBankManagement(actorUsername, actorPassword) {
    const user = verifyOfficer(actorUsername, actorPassword);
    if (!user) return null;
    if (user.role === 'ADMIN') return user;
    if (user.agency === 'NGÂN HÀNG' && user.role === 'LÃNH ĐẠO') return user;
    return null;
}
function calcOfficerPayrollEntry(officer) {
    const base = officer.baseSalary || BASE_SALARY_DEFAULT;
    const pos = officer.positiveRatings || 0;
    const neg = officer.negativeRatings || 0;
    const kpiScore = Math.max(0, Math.min(100, 60 + pos * 8 - neg * 12));
    const bonus = Math.round(base * (kpiScore / 100) * 0.15);
    const deduction = neg * 400000;
    const total = Math.max(0, base + bonus - deduction);
    return { username: officer.username, displayName: officer.displayName, agency: officer.agency, role: officer.role, baseSalary: base, bonus, deduction, kpiScore, total };
}
...
app.post('/api/applications/action', safeRoute(async (req, res) => {
    ...
    if (action === 'escalate_complaint') {
        ...
        if (application.status !== 'Đã Phê Duyệt' && application.status !== 'Bị Bác Bỏ') {
            return res.status(409).json({ error: 'Chỉ có thể khiếu nại sau khi hồ sơ đã có quyết định phê duyệt hoặc bác bỏ.' });
        }
        if (tier === 1) {
            if (application.escalationTier >= 1) return res.status(409).json({ error: 'Khiếu nại Cấp 1 đã được gửi trước đó.' });
            ...
            application.satisfaction = 'KHÔNG HÀI LÒNG'; application.handler = "Chờ Lãnh Đạo xem xét"; application.handlerTitle = "KHIẾU NẠI";
            ...
            if (application.escalationTier !== 1) return res.status(409).json({ error: 'Phải hoàn tất khiếu nại Cấp 1 trước khi leo thang lên Thanh Tra.' });
            ...
            application.handler = "Chờ Thanh Tra giám sát"; application.handlerTitle = "KHIẾU NẠI CẤP 2";
        ...
        if (systemState.archivedApplications[id]) {
            systemState.applications[id] = application;
    ...
    if (action === 'claim_packet') {
        ...
        application.handler = "Chưa phân phối"; application.handlerTitle = "Trực ban liên thông";
        ...
        if (!tier) return res.status(409).json({ error: 'Hồ sơ này không đang trong quy trình khiếu nại.' });
        if (tier === 1 && !isLead && officer.role !== 'ADMIN') {
            return res.status(403).json({ error: 'Chỉ LÃNH ĐẠO cơ quan thụ lý mới có quyền xử lý khiếu nại Cấp 1.' });
        }
        if (tier === 2 && officer.role !== 'THANH TRA' && officer.role !== 'ADMIN') {
            return res.status(403).json({ error: 'Chỉ THANH TRA hoặc ADMIN mới có quyền xử lý khiếu nại Cấp 2.' });
        }
        const resolution = req.body.resolution || 'XEM XÉT LẠI';
        if (tier === 2 && application.originalAgency) application.agency = application.originalAgency;
        application.status = resolution === 'BÁC KHIẾU NẠI' ? 'Đã Phê Duyệt' : 'Đã Tiếp Nhận Xử Lý';
        application.stamp = resolution === 'BÁC KHIẾU NẠI' ? 'stamp-approved' : 'stamp-forwarded';
        application.complaintResolved = true; application.escalationTier = 0; application.satisfaction = null;
        application.handler = officer.displayName; application.handlerTitle = officer.role;
        application.logs.push({ sender: officerName, msg: `📋 Kết luận khiếu nại Cấp ${tier}: [${resolution}]. Hồ sơ ${resolution === 'BÁC KHIẾU NẠI' ? 'giữ nguyên quyết định ban đầu' : 'được mở lại để thẩm định lại từ đầu'}.`, time: timeNow });
        addLog(`Hồ sơ ${id} — khiếu nại Cấp ${tier} đã được ${officerName} kết luận: ${resolution}.`);
    } else if (action === 'request_archive_return') {
        const canRequest = isLead || officer.role === 'THANH TRA' || officer.role === 'ADMIN';
        if (!canRequest) return res.status(403).json({ error: 'Chỉ LÃNH ĐẠO, THANH TRA hoặc ADMIN mới được yêu cầu trả hồ sơ từ kho lưu trữ.' });
        ...
        systemState.archiveReturnRequests.unshift({ id: reqId, archiveId: id, requester: officer.displayName, requesterRole: officer.role, agency: officer.agency, reason, status: 'CHỜ ADMIN DUYỆT', time: new Date().toLocaleString('vi-VN') });
        addLog(`${officerName} gửi yêu cầu trả hồ sơ [${id}] từ kho lưu trữ — lý do: ${reason}.`);
    } else if (action === 'archive_return') {
        if (officer.role !== 'ADMIN') return res.status(403).json({ error: 'Chỉ ADMIN mới có quyền trả hồ sơ từ kho lưu trữ về luồng xử lý.' });
        application = systemState.archivedApplications[id];
        if (!application) return res.status(404).json({ error: 'Không tìm thấy hồ sơ trong kho lưu trữ.' });
        application.status = "Đã Tiếp Nhận Xử Lý"; application.stamp = "stamp-forwarded";
        application.logs.push({ sender: "Hệ thống số hóa", msg: `ADMIN [${officer.displayName}] trả hồ sơ từ kho lưu trữ về luồng xử lý theo yêu cầu cơ quan giám sát.`, time: timeNow });
        systemState.applications[id] = application; delete systemState.archivedApplications[id];
        systemState.archiveReturnRequests.filter(r => r.archiveId === id && r.status === 'CHỜ ADMIN DUYỆT').forEach(r => { r.status = 'ĐÃ TRẢ HỒ SƠ'; });
        addLog(`ADMIN trả hồ sơ ${id} từ kho lưu trữ về luồng xử lý.`);
    } else if (action === 'archive_delete') {
        if (officer.role !== 'ADMIN') return res.status(403).json({ error: 'Chỉ ADMIN mới có quyền hủy/xóa hồ sơ trong kho lưu trữ.' });
        if (!systemState.archivedApplications[id]) return res.status(404).json({ error: 'Không tìm thấy hồ sơ trong kho lưu trữ.' });
        delete systemState.archivedApplications[id];
        systemState.archiveReturnRequests = systemState.archiveReturnRequests.filter(r => r.archiveId !== id);
        addLog(`ADMIN đã hủy và xóa vĩnh viễn hồ sơ ${id} khỏi kho lưu trữ.`);
        broadcastUpdate();
        return res.json({ success: true });
...
app.post('/api/applications/submit', safeRoute(async (req, res) => {
    ...
    const isComplaintDoc = docType && (docType.includes('KHIẾU NẠI') || docType.includes('TỐ CÁO'));
    systemState.applications[id] = {
        id, sender, agency, docType, content, priority: isComplaintDoc ? 'KHẨN CẤP' : priority,
        status: isComplaintDoc ? "Đang Khiếu Nại Cấp 1 (Lãnh Đạo Trực Tiếp)" : "Đang Chờ Tiếp Nhận",
        stamp: isComplaintDoc ? "stamp-rejected" : "stamp-pending",
        handler: isComplaintDoc ? "Chờ Lãnh Đạo xem xét" : "Chưa phân phối",
        handlerTitle: isComplaintDoc ? "KHIẾU NẠI MỚI" : "Hệ thống tự động",
        logs: [{ sender: "Hệ thống", msg: isComplaintDoc ? "Đơn khiếu nại/tố cáo được tiếp nhận — chuyển thẳng Lãnh Đạo/Thanh Tra xử lý theo quy trình ưu tiên." : "Đơn thư số hóa truyền tải lên trục liên thông thành công.", time: new Date().toLocaleTimeString('vi-VN') }],
        satisfaction: null, escalationTier: isComplaintDoc ? 1 : 0
    };
...
app.post('/api/officer/save', safeRoute(async (req, res) => {
    ...
    systemState.authorizedPersonnel[username] = { username, password, displayName, role, agency, positiveRatings: existingRatings.positiveRatings, negativeRatings: existingRatings.negativeRatings, baseSalary: existingRatings.baseSalary || BASE_SALARY_DEFAULT };
...
app.post('/api/bank/deposit-request', safeRoute(async (req, res) => {
    ...
    if (!note) return res.status(400).json({ error: 'Vui lòng điền biên lai/chứng từ nộp tiền để Ngân Hàng đối chiếu phê duyệt.' });
    if (depositRequests.find(r => r.accountNumber === acc.accountNumber && r.status === 'CHỜ DUYỆT')) {
        return res.status(409).json({ error: 'Bạn đã có yêu cầu nạp tiền đang chờ duyệt.' });
    }
    const id = `NT-${Math.floor(1000 + Math.random() * 9000)}`;
    depositRequests.unshift({ id, accountNumber: acc.accountNumber, username: acc.username, amount, method, note, status: 'CHỜ DUYỆT', time: new Date().toLocaleString('vi-VN') });
    addLog(`Công dân @${acc.username} gửi biểu mẫu yêu cầu nạp tiền [${id}]: ${amount.toLocaleString('vi-VN')} VND — chờ Ngân Hàng phê duyệt.`);
    res.json({ success: true, id });
}));
...
app.post('/api/bank/deposit-decide', safeRoute(async (req, res) => {
    ...
    const entry = depositRequests.find(r => r.id === req.body.id);
    if (!entry) return res.status(404).json({ error: 'Không tìm thấy yêu cầu nạp tiền.' });
    if (entry.status !== 'CHỜ DUYỆT') return res.status(409).json({ error: 'Yêu cầu này đã được xử lý.' });
    const decision = req.body.decision;
    entry.decidedBy = staff.displayName;
    if (decision === 'DUYỆT') {
        const acc = findAccountByNumber(entry.accountNumber);
        ...
        logBankTx({ id: txId, accountNumber: acc.accountNumber, type: 'NẠP TIỀN (ĐÃ DUYỆT)', amount: entry.amount, counterpart: entry.method, note: entry.note, time: new Date().toLocaleString('vi-VN'), status: 'THÀNH CÔNG', handledBy: staff.displayName });
        addLog(`${staff.displayName} DUYỆT nạp tiền [${entry.id}] — cộng ${entry.amount.toLocaleString('vi-VN')} VND vào TK [${acc.accountNumber}].`);
        res.json({ success: true, billId: txId });
    } else {
        entry.status = 'TỪ CHỐI';
        addLog(`${staff.displayName} TỪ CHỐI yêu cầu nạp tiền [${entry.id}] của @${entry.username}.`);
        res.json({ success: true });
    }
}));

app.post('/api/bank/deposit', safeRoute(async (req, res) => {
    return res.status(403).json({ error: 'Nạp tiền tự động đã bị khoá. Vui lòng gửi biểu mẫu yêu cầu nạp tiền để Ngân Hàng phê duyệt.' });
}));
...
app.post('/api/bank/staff-state', safeRoute(async (req, res) => {
    ...
    res.json({ success: true, accounts, transactions: bankTransactions.slice(0, 300), loans: loanRequests, accountRequests: bankAccountRequests, depositRequests, salaryPayrollRuns, treasuryBalance: bankAccounts['__treasury__'].balance, interestRate: SAVINGS_INTEREST_RATE });
}));

// [LÃNH ĐẠO NGÂN HÀNG / ADMIN] Lập bảng lương theo KPI cán bộ
app.post('/api/bank/salary/create', safeRoute(async (req, res) => {
    const mgr = verifyBankManagement(req.body.actorUsername, req.body.actorPassword);
    if (!mgr) return res.status(403).json({ error: 'Chỉ Lãnh Đạo Ngân Hàng hoặc ADMIN mới được lập bảng lương.' });
    const period = (req.body.period || new Date().toLocaleDateString('vi-VN', { month: 'long', year: 'numeric' })).trim();
    const officers = Object.values(systemState.authorizedPersonnel).filter(o => o.username !== 'admin' && o.role !== 'ADMIN' && (o.baseSalary || 0) > 0);
    const entries = officers.map(calcOfficerPayrollEntry);
    const id = `BL-${Math.floor(1000 + Math.random() * 9000)}`;
    salaryPayrollRuns.unshift({ id, period, status: 'CHỜ ADMIN PHÊ DUYỆT', createdBy: mgr.displayName, approvedBy: null, time: new Date().toLocaleString('vi-VN'), entries });
    addLog(`${mgr.displayName} lập bảng lương [${id}] kỳ [${period}] — ${entries.length} cán bộ, tổng chi: ${entries.reduce((s, e) => s + e.total, 0).toLocaleString('vi-VN')} VND. Chờ ADMIN phê duyệt.`);
    res.json({ success: true, id, total: entries.reduce((s, e) => s + e.total, 0) });
}));

// [ADMIN] Phê duyệt/từ chối chi lương
app.post('/api/bank/salary/decide', safeRoute(async (req, res) => {
    const actor = verifyOfficer(req.body.actorUsername, req.body.actorPassword);
    if (!actor || actor.role !== 'ADMIN') return res.status(403).json({ error: 'Chỉ ADMIN mới có quyền phê duyệt chi lương.' });
    const run = salaryPayrollRuns.find(r => r.id === req.body.id);
    if (!run) return res.status(404).json({ error: 'Không tìm thấy bảng lương.' });
    if (run.status !== 'CHỜ ADMIN PHÊ DUYỆT') return res.status(409).json({ error: 'Bảng lương này đã được xử lý.' });
    const decision = req.body.decision;
    if (decision === 'TỪ CHỐI') {
        run.status = 'TỪ CHỐI'; run.approvedBy = actor.displayName;
        addLog(`ADMIN từ chối bảng lương [${run.id}].`);
        return res.json({ success: true });
    }
    const treasury = bankAccounts['__treasury__'];
    const totalPay = run.entries.reduce((s, e) => s + e.total, 0);
    if (treasury.balance < totalPay) {
        return res.status(400).json({ error: `Kho Bạc không đủ chi trả. Cần ${totalPay.toLocaleString('vi-VN')} VND, hiện có ${treasury.balance.toLocaleString('vi-VN')} VND.` });
    }
    treasury.balance -= totalPay;
    run.entries.forEach(entry => {
        entry.paid = true;
        const acc = bankAccounts[entry.username];
        if (acc) {
            acc.balance += entry.total;
            const txId = `LUONG-${Date.now().toString().slice(-6)}-${entry.username.slice(0, 3)}`;
            logBankTx({ id: txId, accountNumber: acc.accountNumber, type: 'NHẬN LƯƠNG & PHÚC LỢI', amount: entry.total, counterpart: 'KHO BẠC NHÀ NƯỚC', note: `Kỳ ${run.period} | LCB: ${entry.baseSalary.toLocaleString('vi-VN')} + Thưởng: ${entry.bonus.toLocaleString('vi-VN')} - Trừ VP: ${entry.deduction.toLocaleString('vi-VN')}`, time: new Date().toLocaleString('vi-VN'), status: 'THÀNH CÔNG', handledBy: actor.displayName, payrollId: run.id, payrollDetail: entry });
        }
    });
    run.status = 'ĐÃ CHI TRẢ'; run.approvedBy = actor.displayName; run.paidAt = new Date().toLocaleString('vi-VN');
    addLog(`ADMIN phê duyệt chi lương [${run.id}] kỳ [${run.period}] — tổng ${totalPay.toLocaleString('vi-VN')} VND từ Kho Bạc.`);
    res.json({ success: true, totalPaid: totalPay });
}));

app.post('/api/bank/salary/payslip', safeRoute(async (req, res) => {
    const acc = verifyBankAccount(req.body.accountNumber, req.body.pin);
    if (!acc) return res.status(401).json({ error: 'Phiên đăng nhập không hợp lệ.' });
    const slips = bankTransactions.filter(t => t.accountNumber === acc.accountNumber && t.type === 'NHẬN LƯƠNG & PHÚC LỢI' && t.payrollDetail);
    res.json({ success: true, payslips: slips.slice(0, 20) });
}));
...
server.listen(PORT, () => {
    console.log(`[REALTIME CENTRAL V14.0] RUNNING PERFECTLY ON PORT ${PORT}`);
    console.log(`[NGÂN HÀNG SỐ v14] Nạp tiền qua biểu mẫu, lãi TK ${(SAVINGS_INTEREST_RATE*100).toFixed(1)}%/kỳ, bảng lương KPI: SẴN SÀNG.`);
